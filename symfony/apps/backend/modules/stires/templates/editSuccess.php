<h4 class="mb-3">Editeaza Stire</h4>

<div class="clear"></div>

<form action="<?php echo url_for('@default?module=stires&action=edit');?>?lid=<?php echo $form->getObject()->getLibraryId();?>" method="post" enctype="multipart/form-data">
...
<table class="span-19">
	...
	<tr>
    	<th>Poza</th>
        <td><img src="<?php echo filmsiStirePhotoThumb($form->getObject()->getFilename());?>" /><br /><?php echo $form['filename']->render();?><br /><?php echo $form['filename']->renderError();?></td>
    </tr>
	<tr>
    	<th>Publicat la</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#stire_publish_date').datepicker({dateFormat: 'yy-mm-dd'});
		$('#stire_expiration_date').datepicker({dateFormat: 'yy-mm-dd'});
		...
		$("#photo-album-selector").autocomplete({
		  ...
		  select: function(event, ui){
			  $('#stire_photo_album_id').val(ui.item.value);
		...
		$("#video-album-selector").autocomplete({
		  ...
		  select: function(event, ui){
			  $('#stire_video_album_id').val(ui.item.value);
